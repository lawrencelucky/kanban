...
const OrganizationAuth = require('../models/OrganizationAuth');
const { BadRequestError, UnauthenticatedError } = require('../errors');

const registerOrganization = async (req, res) => {
  const { email, password, name, description } = req.body;

  const organizationAlreadyExists = await OrganizationAuth.findOne({ email });
  ...
  const organization = await OrganizationAuth.create({
    email,
    password,
    name,
    description,
...
const loginToOrganization = async (req, res) => {
  ...
  const organization = await OrganizationAuth.findOne({ email });
